...
<div class="absolute bottom-5 right-5 w-100 shadow-2xl overflow-hidden">
...
stateDiagram-v2
    ...
    state "even , even" as EE
    state "odd , even" as OE
    state "even , odd" as EO
    state "odd , odd" as OO
...
<v-click>
By using native fast Boolean `XOR` logic over a bounded size, Z3 reduces the problem to an exact pure SAT solve that finishes in **< 0.1s**.
</v-click>
